...
class AddItemTestCase(TestCase):
    ...
    def add_file_request(self, data, expected_response):
        ...
        if expected_response['status'] == 'ok':
            self.assertTrue(File.objects.filter(path=expected_response['path']).exists())
        else:
    ...
    def add_directory_request(self, data, expected_response):
        ...
        self.assertJSONEqual(response.content, expected_response)
        if expected_response['status'] == 'ok':
            self.assertTrue(Directory.objects.filter(path=expected_response['path']).exists())
        else:
...
class SaveFileTestCase(TestCase):
    ...
    def save_file_request(self, data, expected_response):
        ...
        self.assertJSONEqual(response.content, expected_response)
        if expected_response['status'] == 'ok':
            self.assertEqual(File.objects.get(path=data['path']).content, data['content'])
...
class CompileFileTestCase(TestCase):
    ...
    def compile_file_request(self, data, expected_response):
        ...
        self.assertJSONEqual(response.content, expected_response)
        if expected_response['status'] == 'ok':
            self.assertTrue(File.objects.filter(path=data['file'][:-2] + ".asm").exists())
            self.assertTrue(File.objects.get(path=data['file'][:-2] + ".asm").owner == self.user)
            self.assertTrue(File.objects.get(path=data['file'][:-2] + ".asm").parent == self.dir_home)
            self.assertTrue(File.objects.get(path=data['file'][:-2] + ".asm").content != '')
            self.assertTrue(File.objects.get(path=data['file'][:-2] + ".asm").availability)
